...
public class BoardDAO {
	// Data Access Object (DAO) => my_board테이블에 관련된 정보 처리 객체
	...
	// 사용시 연결정보(pool) 대여, 사용 후 반납처리 (기본처리 로직은 대기->생성 후 반납)

	// 1. 라이브러리 설치
	// 2. context.xml 파일 생성(디비연결정보)
	// 3. 파일(2번파일)을 호출해서 디비 연결
	...
	public void insertBoard(BoardBean bb) {
		...
		try {
			...
			// (화살표 모양)
			// select num from my_board;
			// rs.next : false
			// => 함수의 결과가 데이터의 유무에 따라서 커서유무 결정
			// (원모양)

			// 게시판 번호 계산
			String sql = "select max(num) from my_board";
			...
			// 3. sql작성(insert) & pstmt 객체 생성
			sql = "insert into my_board(num,name,pass,subject,content,"
			...
			pstmt.setInt(8, 0); // re_lev 답글 들여쓰기 / 일반글은 들여쓰기 없음 (0)
	...
	public int getBoardCount() {
		...
		try {
			...
			// 글의 총 개수를 계산
			sql = "select count(num) from my_board";
	...
	public BoardBean getBoard(int num) {
		...
		try {
			// 1.2 디비 연결
			getCon();
			// 3. sql (num에 해당하는 모든 글의 정보 가져오기) & pstmt객체
	...
	public int updateBoard(BoardBean bb) {
		...
		try {
			// 1.2 디비연결
			con = getCon();
			// 3. sql 작성 (글이 있는지 판단 후 글 수정) & pstmt 객체
			...
			if (rs.next()) { // 데이터 있을때
				if (bb.getPass().equals(rs.getString("pass"))) {
					...
					sql = "update my_board set name=?,subject=?,content=? where num=?";
					pstmt = con.prepareStatement(sql);
...
// reInsertBoard(bb)--------------------------------------------------
	...
	public void reInsertBoard(BoardBean bb){
		int num = 0;  //-> 글 번호를 저장하는 변수 
		
		try {
			con = getCon();  // -> 연결정보를 con에 저장
			...
			rs = pstmt.executeQuery();  // -> rs에는 num이 저장될것임
 			if(rs.next()){
 				num = rs.getInt(1)+1; 
 			}
			...
			// 답글 순서 재배치 (update)
 			...
 			sql = "update my_board set re_seq = re_seq + 1 " 
			...
			// 답글을 저장 (insert)
 			...
 			sql="insert into my_board values(?,?,?,?,?,?,?,?,?,now(),?,?)";
			...
			pstmt.setInt(9, bb.getRe_seq()+1); // re_seq : 순서대로 기존의 값 + 1
			pstmt.setString(10, bb.getIp());
			pstmt.setString(11, bb.getFile());
